...
for epoch in range(num_epochs_base):
    epoch_loss = 0.0
    for batch_idx, (x_batch, f_batch, v_batch, y_batch) in enumerate(train_loader):
        ...
        # Log the computational graph for the first batch of the first epoch
        # Skipping add_graph to avoid tracing issues
        if epoch == 0 and batch_idx == 0:
            print("Skipping add_graph due to potential tracing issues.")
...
# %% Example Usage OF THE PLOTTING AND DEBUGGING FUNCTION FRO BASE MODEL:
...
# base_predictor = load_base_predictor(BasePredictorClass, model_name, device)
# ✅ Modelo guardado en: ./saved_models/TestPSpyt_otres_2025_03_24_22_41_base_model.pth
# ✅ Modelo guardado en: ./saved_models/TestPSpyt_otres_2025_03_25_02_00_base_model.pth
# %%

save_base_predictor(base_predictor, f'{model_name}_ar_4')
# %%

save_base_predictor(base_predictor, f'{model_name}_ar_5')
# saved_models/TestPSpyt_otres_2025_03_26_17_49_ar_5_base_model.pth

# %%

save_base_predictor(base_predictor, f'{model_name}_ar_6')
# saved_models/TestPSpyt_otres_2025_03_27_05_25_ar_6_base_model.pth
# %%
save_base_predictor(base_predictor, f'{model_name}_ar_7')
# saved_models/TestPSpyt_otres_2025_03_27_05_25_ar_7_base_model.pth


# %%
##########################################################
# TEST PLOTING OF HOW THE BASE MODEL PREDICTS
##########################################################
plot_time_series_predictions(
    base_predictor,
    X_df_train_ar_tensor,
...
##########################################################
# DEBBUGING FUNS
# Código adicional para ayudar a depurar dimensiones:

def print_dimensions(dataset_loader):
    """
    Ayuda a visualizar las dimensiones de los datos para depuración.
    """
    for i, (x_ar, x_flag, x_var, y) in enumerate(dataset_loader):
        print(f"Batch {i}:")
        print(f"  x_ar shape: {x_ar.shape}")
        print(f"  x_flag shape: {x_flag.shape}")
        print(f"  x_var shape: {x_var.shape}")
        print(f"  y shape: {y.shape}")

        if i >= 2:  # Solo imprime algunos lotes
            break


# Para verificar las dimensiones del modelo base:


def verify_model_dimensions(base_predictor, x_ar, x_flag, x_var):
    """
    Verifica si las dimensiones son compatibles con el modelo base.
    """
    try:
        output = base_predictor(x_ar, x_flag, x_var)
        print(f"¡Éxito! El modelo base puede procesar las entradas correctamente.")
        print(f"  Entrada AR: {x_ar.shape}")
        print(f"  Entrada flag: {x_flag.shape}")
        print(f"  Entrada var: {x_var.shape}")
        print(f"  Salida: {output.shape}")
        return True
    except Exception as e:
        print(f"Error al ejecutar el modelo base: {e}")
        return False

# %%
# TESTING CHANGES ON AR MODEL PARAMETERS
# learning_rate_base = 0.0005
# num_epochs_base = 5000
# patience_base = 600

# Some training parameters
learning_rate_ar = 0.00005
num_epochs_ar = 5000
patience_ar = 600
num_hours_to_forecast  = 4

# %%
# CLASS FOR THE AUTOREGRESSIVE MODEL
# Comentando esta parte para probar el modelo, en principio funcionan iguales..
...
class AutoregressivePredictor(nn.Module):
    ...
    # def update_inputs(self, preds, X_data, F_data, V_data, current_idx, batch_size):
    #     """
    #     Updates the input sequences for the next autoregressive step.
...
# %%
##########################################################    
# LOAD OF BASE MODEL PRETRAINED WEIGHTS
##########################################################    

# model_path_base = "/OZONO/HOME/pedro/git2/gitflow/air_pollution_forecast/saved_models/TestPSpyt_otres_2025_03_24_22_41_base_model.pth"
model_path_base = "/OZONO/HOME/pedro/git2/gitflow/air_pollution_forecast/saved_models/TestPSpyt_otres_2025_03_25_02_00_base_model.pth"
# model = base_predictor_class().to(device)  # Inicializar la arquitectura
model_path_base = "/OZONO/HOME/pedro/git2/gitflow/air_pollution_forecast/saved_models/TestPSpyt_otres_2025_03_25_02_00_ar_4_base_model.pth"
# saved_models/TestPSpyt_otres_2025_03_26_17_49_ar_5_base_model.pth
...
#"TestPSpyt_otres_2025_03_25_02_00_base_model.pth"
base_predictor = BasePredictor(
    num_ar_features, num_flag_features, num_var_features, output_dim, sequence_length
).to(device)
...
# %%
##########################################################    
# HASTA AQUÍ LLEGA REFACTORIZACIÓN HASTA EL MOMEMENTO
##########################################################    
# ya se puede entrenar modelo base, modelo ar y guardar los pesos de modelo base
# después de haber realizado entrenamientos. 
# Modelo base se ve que funciona más menos, pero ha habido problemas con el modelo AR 
# a 48 horas no aprendió nada, por lo que se va a probar con 4 horas, para ver si funciona.
# Esta tardando en aprender cada paso de depuración.
# Next: preparación de scripts o notebooks, para mandarlas a entrenar en background, en paralelo. 
##########################################################    
# # %%
# # 

# def evaluate_and_plot_ar_predictions(
#     ar_predictor,
#     X_data,
#     Y_data,
#     sequence_start_idx,
...
#     Args:
#         ar_predictor: The trained AutoregressivePredictor model.
#         X_data: The input data tensor (X_df_train_tensor or X_df_val_tensor).
#         Y_data: The target data tensor (Y_df_train_tensor or Y_df_val_tensor).
#         sequence_start_idx: The starting index of the sequence to evaluate.
#         num_hours_to_forecast: The number of hours to forecast.
#         sequence_length: The length of the input sequence.
#         device: The device to use ('cuda' or 'cpu').
...
#     # Check if the sequence fits in the dataset:
#     if sequence_start_idx + sequence_length + num_hours_to_forecast > len(X_data):
#         print(
#             f"Error: sequence_start_idx {sequence_start_idx} + sequence_length {sequence_length} + num_hours_to_forecast {num_hours_to_forecast} exceeds dataset length {len(X_data)}"
#         )
#         return

#     # Create the input sequence
#     # input_sequence = X_data[sequence_start_idx:sequence_start_idx + sequence_length, :]
#     # input_sequence_flat = input_sequence.reshape(-1).unsqueeze(0).to(device)  # Flatten and add batch dimension

#     # Create the input sequence
#     input_sequence_ar = X_data[
#         sequence_start_idx : sequence_start_idx + sequence_length, :num_ar_features
#     ]
#     input_sequence_flag = X_data[
#         sequence_start_idx : sequence_start_idx + sequence_length,
#         num_ar_features : num_ar_features + num_flag_features,
#     ]
#     input_sequence_var = X_data[
#         sequence_start_idx : sequence_start_idx + sequence_length, -num_var_features:
#     ]

#     # Flatten the AR sequence
#     # Shape: (sequence_length * num_ar_features,)
#     input_sequence_ar_flat = input_sequence_ar.reshape(-1)

#     # Flatten the flag sequence
#     # Shape (sequence_length * num_flag_features)
#     input_sequence_flag_flat = input_sequence_flag.reshape(-1)

#     # Flatten the var sequence
#     # Shape (sequence_length * num_flag_features)
#     input_sequence_var_flat = input_sequence_var.reshape(-1)

#     # Concatenate the flattened AR sequence with the flattened flag sequence and the add1 var
#     input_sequence_flat = (
#         torch.cat(
#             (input_sequence_ar_flat, input_sequence_flag_flat, input_sequence_var_flat)
#         )
#         .unsqueeze(0)
#         .to(device)
#     )  # Shape: (sequence_length * num_ar_features + sequence_length * num_flag_features + num_var_features,)
...
# # Example Usage:
# columns_to_plot = [0, 2, 5, 105]  # Example columns to plot
# sequence_start_idx = 10  # Example starting index

# # Evaluate and plot on the training data
# evaluate_and_plot_ar_predictions(
#     ar_predictor,
#     X_df_train_tensor,
#     Y_df_train_tensor,
#     sequence_start_idx,
...
# # Puedes cambiar este índice para probar diferentes partes del set de entrenamiento
# sequence_start_idx = 0

# # Crear la secuencia de entrada
# input_sequence = X_df_train_tensor[
#     sequence_start_idx : sequence_start_idx + sequence_length, :
# ]
...
# train_ar_dataset = AutoregressiveTimeSeriesDataset(
#     X_df_train_tensor, Y_df_train_tensor, sequence_length, num_hours_to_forecast
# )
# train_ar_loader = DataLoader(train_ar_dataset, batch_size=batch_size, shuffle=True)

# # Crear el validation DataLoader
# val_ar_dataset = AutoregressiveTimeSeriesDataset(
#     X_df_val_tensor, Y_df_val_tensor, sequence_length, num_hours_to_forecast
# )
# val_ar_loader = DataLoader(val_ar_dataset, batch_size=batch_size, shuffle=False)

# # Verificar la forma del output
# example_X, example_Y = train_ar_dataset[0]
# # Should be: (sequence_length * num_features,)
# print(f"Shape of X_sequence_flat: {example_X.shape}")
# # Should be: (num_hours_to_forecast, num_features)
# print(f"Shape of Y_targets: {example_Y.shape}")
...
#     """
#     # Crear la secuencia de entrada
#     input_sequence = X_data[
#         sequence_start_idx : sequence_start_idx + sequence_length, :
#     ]
#     # Aplanamos y añadimos dimensión para batch
#     input_sequence_flat = input_sequence.reshape(-1).unsqueeze(0).to(device)
...
#         plt.plot(
#             actual_np[:, col_idx], label=f"True Columna {col_idx}", linestyle="dashed"
#         )
...
# ar_predictor = AutoregressivePredictor(
#     base_predictor, input_dim, output_dim, sequence_length, num_hours_to_forecast
# ).to(device)
...
# # Initialize TensorBoard writer with model_name and suffix 'ar_model'
# log_dir = f"./tensorboard_logs/{model_name}_ar_model"
# writer = SummaryWriter(log_dir=log_dir)
...
#         if (epoch + 1) % 10 == 0:
#             print(
#                 f"Epoch [{epoch + 1}/{num_epochs_ar}], Loss: {epoch_loss/len(train_ar_loader):.4f}, Val Loss: {val_loss:.4f}"
#             )
...
#         if epochs_no_improve >= patience_ar:
